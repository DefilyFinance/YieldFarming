...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 DFLReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        DFL.mint(devAddress, DFLReward.mul(10).div(100));
		DFL.mint(treasuryAddress, DFLReward.mul(10).div(100));
        DFL.mint(address(this), DFLReward);
        // Automatically burn 2% of minted tokens
        DFL.mint(BURN_ADDRESS, DFLReward.mul(50).div(1000));
