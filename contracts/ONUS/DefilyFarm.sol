...
contract Defily is ERC20('Defily Token', 'DFL') {
...
// MasterChef is the master of dragons. He can make DFL and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once DFL is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accRewardPerShare;   // Accumulated Yieldes per share, times 1e18. See below.
    ...
    // The DFL TOKEN!
    Defily public immutable DFL;
    address public devAddress;
	address public treasuryAddress;
    address public feeAddress;
    

    // DFL tokens created per block.
    uint256 public rewardPerBlock = 8680555555555555;
    
    uint256 constant MAX_REWARD_PERBLOCK= 10 ether;
    ...
    // The block number when DFL mining starts.
    ...
    // Update the given pool's DFL allocation point and withdraw fee. Can only be called by the owner.
    ...
    // View function to see pending DFL on frontend.
    function pendingRewards(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0 && totalAllocPoint>0) {
            ...
            uint256 DFLReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accRewardPerShare = accRewardPerShare.add(DFLReward.mul(1e18).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 DFLReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        DFL.transfer(devAddress, DFLReward.mul(100).div(1000));
		DFL.transfer(treasuryAddress, DFLReward.mul(100).div(1000));
        // Automatically burn 5% of minted tokens
        // DFL.transfer(BURN_ADDRESS, DFLReward.mul(50).div(1000));
        pool.accRewardPerShare = pool.accRewardPerShare.add(DFLReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for DFL allocation.
    ...
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        uint256 DFLBal = DFL.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > DFLBal) {
            transferSuccess = DFL.transfer(_to, DFLBal);
        } else {
            transferSuccess = DFL.transfer(_to, _amount);
        }
    ...
    function updateEmissionRate(uint256 _rewardPerBlock) external onlyOwner {
        require(_rewardPerBlock<=MAX_REWARD_PERBLOCK,"Too high");
        massUpdatePools();
